...
namespace HomeAccountingSystem.AccountManagement
{
    public partial class LoanAccountsForm : Form
    {
        ...
        private void LoanAccountsForm_Load(object sender, EventArgs e)
        {
        ...
        {
            this.gridControlDataList.DataSource = LoanAccountsManager.Instance.getLoanAccountsData(this.dateTimeInputStartDate.Value, this.dateTimeInputEndDate.Value);
            this.selectRow();
        }
        ...
        private void buttonXDelete_Click(object sender, EventArgs e)
        {
            ...
            int selectRow = this.gridViewDataList.GetSelectedRows()[0];
            int pk = Convert.ToInt32(this.gridViewDataList.GetRowCellValue(selectRow, "pk").ToString());
            bool isSuccess = LoanAccountsManager.Instance.Delete(pk);
            if (isSuccess)
            {
