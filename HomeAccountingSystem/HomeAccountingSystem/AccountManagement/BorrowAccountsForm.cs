...
namespace HomeAccountingSystem.AccountManagement
{
    public partial class BorrowAccountsForm : Form
    {
        ...
        private void BorrowAccountsForm_Load(object sender, EventArgs e)
        {
            this.comboBoxExTIME.SelectedIndex = 1;
            loadDataList();
        }
        private void loadDataList()
        {
            this.gridControlDataList.DataSource = BorrowAccountsManager.Instance.getBorrowAccountsData(this.dateTimeInputStartDate.Value, this.dateTimeInputEndDate.Value);
            this.selectRow();
        }
        ...
        private void buttonXModify_Click(object sender, EventArgs e)
        {
            ...
            jt_jr_zm zczmModel = BorrowAccountsManager.Instance.GetModel(pk);
            EditBorrowAccountsForm form = new EditBorrowAccountsForm();
            form.m_jrzmModel = zczmModel;
            form.ShowDialog();
        ...
        private void buttonXDelete_Click(object sender, EventArgs e)
        {
            ...
            int selectRow = this.gridViewDataList.GetSelectedRows()[0];
            int pk = Convert.ToInt32(this.gridViewDataList.GetRowCellValue(selectRow, "pk").ToString());
            bool isSuccess = BorrowAccountsManager.Instance.Delete(pk);
            if (isSuccess)
            {
